...
STADIUM_DATA={
    "Yankee Stadium":("Bronx","NY",1.18),"Citizens Bank Park":("Philadelphia","PA",1.20),
    "Great American Ball Park":("Cincinnati","OH",1.25),"Coors Field":("Denver","CO",1.30),
    "Dodger Stadium":("Los Angeles","CA",1.05),"Truist Park":("Atlanta","GA",1.06),
    "Fenway Park":("Boston","MA",1.03),"Citi Field":("Queens","NY",0.96),
    "Oracle Park":("San Francisco","CA",0.82),"Petco Park":("San Diego","CA",0.93),
    "T-Mobile Park":("Seattle","WA",0.91),"Kauffman Stadium":("Kansas City","MO",0.88),
    "Wrigley Field":("Chicago","IL",1.00),"Guaranteed Rate Field":("Chicago","IL",1.12),
    "Oriole Park at Camden Yards":("Baltimore","MD",1.02),"Rogers Centre":("Toronto","ON",1.05),
    "Tropicana Field":("St. Petersburg","FL",0.92),"Progressive Field":("Cleveland","OH",0.97),
    "Comerica Park":("Detroit","MI",0.90),"Target Field":("Minneapolis","MN",0.98),
    "Minute Maid Park":("Houston","TX",1.04),"Angel Stadium":("Anaheim","CA",0.96),
    "Globe Life Field":("Arlington","TX",1.00),"loanDepot park":("Miami","FL",0.92),
    "Nationals Park":("Washington","DC",1.02),"American Family Field":("Milwaukee","WI",1.08),
    "PNC Park":("Pittsburgh","PA",0.93),"Busch Stadium":("St. Louis","MO",0.95),
...
def get_weather(city,state):
    ...
    try:
        ...
        return {"temp":"N/A","wind":"N/A","dir":"N/A","factor":1.0,"note":"⚖️ weather neutral"}
    factor=1.0; note="⚖️ weather neutral"
    if temp >= 80:
        factor += .05; note="🔥 warm air boost"
    elif temp <= 55:
        factor -= .05; note="❄️ cold air downgrade"
    if wind >= 10:
        factor += .04; note += f" • wind {direction} {wind}mph"
...
def get_schedule():
    ...
    for d in data.get("dates",[]):
        for g in d.get("games",[]):
            games.append({
                "gamePk":g.get("gamePk"),"away":g["teams"]["away"]["team"]["name"],"home":g["teams"]["home"]["team"]["name"],
                ...
                "park":g.get("venue",{}).get("name",""),"status":g.get("status",{}).get("detailedState","")})
    return games
...
def build_roster():
    ...
    except: return pd.DataFrame(columns=["name","team","_norm"])
    for t in teams:
        team=normalize_team(t.get("name","")); tid=t.get("id")
...
def pitcher_live(pid):
    if not pid: return {"k_rate":.22,"k9":8.0,"era":4.20,"whip":1.30,"hr9":1.10,"hand":"R"}
    try:
        season=time.strftime("%Y")
        bio=requests.get(f"https://statsapi.mlb.com/api/v1/people/{pid}",timeout=20).json()
        hand=bio.get("people",[{}])[0].get("pitchHand",{}).get("code","R")
        ...
        if not splits: return {"k_rate":.22,"k9":8.0,"era":4.20,"whip":1.30,"hr9":1.10,"hand":hand}
        s=splits[0].get("stat",{})
        ...
        return {"k_rate":so/bf if bf>0 else .22,"k9":(so/ip)*9 if ip>0 else 8.0,"era":safe_float(s.get("era",4.20)),"whip":safe_float(s.get("whip",1.30)),"hr9":(hr/ip)*9 if ip>0 else 1.10,"hand":hand}
    except:
...
hr_leaders_df=get_hr_leaders(60)
if not hr_leaders_df.empty:
    existing=set(batters["_name"].astype(str).apply(norm))
    missing_leaders=hr_leaders_df[~hr_leaders_df["_name"].astype(str).apply(norm).isin(existing)].copy()
    if not missing_leaders.empty:
        batters=pd.concat([batters,missing_leaders],ignore_index=True,sort=False)
...
def batter_metrics(row):
    pa=safe_float(row[b_pa],250) if b_pa else 250; bip=safe_float(row[b_bip],150) if b_bip else 150
    ba=safe_float(row[b_ba],.245) if b_ba else .245; est_ba=safe_float(row[b_est_ba],ba) if b_est_ba else ba
    slg=safe_float(row[b_slg],.400) if b_slg else .400; est_slg=safe_float(row[b_est_slg],slg) if b_est_slg else slg
    woba=safe_float(row[b_woba],.310) if b_woba else .310; est_woba=safe_float(row[b_est_woba],woba) if b_est_woba else woba
    iso=safe_float(row[b_iso],est_slg-est_ba) if b_iso else est_slg-est_ba
    barrel=safe_float(row[b_barrel],None) if b_barrel else None; hard=safe_float(row[b_hard],None) if b_hard else None
    season_hr=safe_float(row[b_season_hr],0) if b_season_hr else 0; hr_rank=safe_float(row[b_hr_rank],999) if b_hr_rank else 999
    injected= bool(row[b_injected]) if b_injected and str(row.get(b_injected,"")).lower() in ["true","1","yes"] else False
    ...
    if season_hr>0:
        power=clamp(power*.78+scale01(season_hr,5,25)*.16+(1-scale01(hr_rank,1,60))*.06,0,1)
...
def auto_matchup_edge(m,live,park_edge,weather_edge,lineup_edge):
    ...
    raw=.34*m["Power"]+.18*m["Laser"]+.13*m["Form Score"]+.18*pitcher_hr+.07*park_edge+.05*weather_edge+.05*lineup_edge-.10*pitcher_k
    edge=clamp(raw,0,1)
    return edge, f"auto edge from batter power + pitcher HR risk + K risk; pitcher hand {live.get('hand','R')} • K risk {round(pitcher_k,2)}"
...
def score_row(player_name,team,matchup,pitcher_name,pitcher_id,park,game_status,game_pk,order="—",lineup="Projected"):
    ...
    m=batter_metrics(b); live=pitcher_live(pitcher_id); pr=pitcher_risk(live)
    city,state,park_factor=stadium_info(park); weather=get_weather(city,state)
    park_edge=scale01(park_factor,.82,1.30); weather_edge=scale01(weather["factor"],.88,1.18); lineup_edge=1-scale01(order,1,9) if isinstance(order,int) else .50
    me,me_note=auto_matchup_edge(m,live,park_edge,weather_edge,lineup_edge)
    dinger_score=round(clamp(10*m["Power"]+6*m["Laser"]+5*m["Contact"]+4*m["Form Score"]+7*me+4*pr+3*park_edge+3*weather_edge+2*lineup_edge,0,42),1)
    hr_prob=clamp(.035+(.26*m["Power"]+.17*m["Laser"]+.11*m["Form Score"]+.20*me+.13*pr+.07*park_edge+.04*weather_edge+.02*lineup_edge)*.34,.010,.40)
    hit_prob=clamp(.28+m["Contact"]*.42,.18,.82); tb_prob=clamp(.20+(m["Power"]*.43+m["Contact"]*.22+m["Laser"]*.20+me*.15)*.48,.10,.76)
    rbi_prob=clamp(.12+(m["Power"]*.43+m["Laser"]*.17+pr*.25+me*.15)*.42,.06,.62); laser_prob=clamp(.15+m["Laser"]*.62+me*.08,.10,.82)
    reasons=(f"{m['Form']} • Auto Matchup Edge {round(me,2)} ({me_note}) • Matchup: {matchup} vs {pitcher_name} • Game Status: {game_status} • Pitcher Risk {round(pr,2)} HR/9 {round(live['hr9'],2)} ERA {live['era']} WHIP {live['whip']} • Park: {park} {park_note(park_factor)} ({park_factor}) • Weather: {weather['note']} {weather['temp']}°F wind {weather['wind']}mph {weather['dir']} • Power {m['Power']} • Laser {m['Laser']} • estSLG {m['estSLG']} • ISO {m['ISO']} • Season HR {m['Season HR']} • HR Rank {m['HR Rank']} • Source {m['Data Source']}")
    return {"Player":player_name,"Team":team,"Matchup":matchup,"Pitcher":pitcher_name,"Park":park,"Game Status":game_status,"Parlay Eligible":"Yes" if is_game_available_for_parlays(game_status) else "No","Lineup":lineup,"Order":order,"Dinger Score":dinger_score,"Grade":grade_score(dinger_score),"Badge":badge_score(dinger_score),"HR %":round(hr_prob*100,1),"Hit %":round(hit_prob*100,1),"TB %":round(tb_prob*100,1),"RBI %":round(rbi_prob*100,1),"Laser %":round(laser_prob*100,1),"Form":m["Form"],"Auto Matchup Edge":round(me,2),"Pitcher Risk":round(pr,2),"Park Edge":round(park_edge,2),"Weather Edge":round(weather_edge,2),"Power":m["Power"],"Laser":m["Laser"],"Season HR":m["Season HR"],"HR Rank":m["HR Rank"],"Data Source":m["Data Source"],"Reasons":reasons}

games_all=get_schedule(); games=[g for g in games_all if not any(x in str(g.get("status","")).lower() for x in ["postponed","cancelled"])]
rows=[]
for g in games:
    matchup=f'{g["away"]} @ {g["home"]}'; status=g.get("status",""); game_pk=g.get("gamePk"); lu=get_lineups(game_pk)
    if g["home_p"]:
        if lu["away"]:
            for h in lu["away"]:
...
if df.empty:
    st.warning("No game rows created. Probable pitchers/lineups may not be posted yet."); st.stop()
...
k_rows=[]
for g in games:
    if not is_game_available_for_parlays(g.get("status","")): continue
    for name,pid,opp in [(g["away_p"],g["away_p_id"],g["home"]),(g["home_p"],g["home_p_id"],g["away"])]:
        if not name: continue
        live=pitcher_live(pid); proj_ks=clamp(3.8+(live["k_rate"]-.20)*18+(live["k9"]-8.0)*.35,2.0,10.5)
        def over_prob(line): return clamp(1/(1+math.exp(-(proj_ks-line))),.05,.92)
        k45=round(over_prob(4.5)*100,1); k55=round(over_prob(5.5)*100,1); k65=round(over_prob(6.5)*100,1); best=max(k45,k55,k65)
        k_rows.append({"Pitcher":name,"Opponent":normalize_team(opp),"Projected Ks":round(proj_ks,1),"Live K%":round(live["k_rate"]*100,1),"K/9":round(live["k9"],1),"ERA":live["era"],"WHIP":live["whip"],"Over 4.5 K%":k45,"Over 5.5 K%":k55,"Over 6.5 K%":k65,"Best K%":best,"Grade":"A+" if best>=75 else "A" if best>=65 else "A-" if best>=58 else "B" if best>=50 else "C","Pick Explanation":f"{name} projects for {round(proj_ks,1)} Ks using live K%, K/9, ERA, and WHIP."})
...
def tier_parlays(data,col,label,name_col="Player"):
    ...
    pool=data.sort_values(col,ascending=False).head(15).reset_index(drop=True); out=[]
    for i in range(0,15,3):
        c=pool.iloc[i:i+3]
        if len(c)<3: continue
        out.append({"Parlay":f"{label} 3-Leg #{len(out)+1}","Leg 1":f"{c.iloc[0][name_col]} ({c.iloc[0][col]}%)","Leg 2":f"{c.iloc[1][name_col]} ({c.iloc[1][col]}%)","Leg 3":f"{c.iloc[2][name_col]} ({c.iloc[2][col]}%)","Avg Model %":round(c[col].mean(),1),"Model Combo Confidence":round((c[col]/100).prod()*100,2),"Notes":f"Dynamic parlay: only games not started yet; group {i+1}-{i+3}"})
    return pd.DataFrame(out)
...
def smart_hr_parlays(data):
    ...
    for i in range(5):
        ...
        if len(legs)==3:
            ...
            parlays.append({"Parlay":f"Smart HR 3-Leg #{i+1}","Leg 1 Anchor":f"{legs[0]['Player']} | {legs[0]['Grade']} | {legs[0]['HR %']}%","Leg 2 Support":f"{legs[1]['Player']} | {legs[1]['Grade']} | {legs[1]['HR %']}%","Leg 3 Value":f"{legs[2]['Player']} | {legs[2]['Grade']} | {legs[2]['HR %']}%","Avg HR %":avg,"Model Combo Confidence":combo,"Strategy":"Dynamic: only games not started; 1 anchor + 1 support + 1 value; different games"})
    return pd.DataFrame(parlays)

def build_rotating_hr_combos(pool):
    if pool is None or pool.empty: return []
    generator_pool=pool.copy()
    generator_pool["Combo Score"]=(generator_pool["HR %"].apply(safe_float)*0.45+generator_pool["Dinger Score"].apply(safe_float)*0.30+generator_pool["Auto Matchup Edge"].apply(safe_float)*25*0.25)
    ...
    for start in range(0,min(len(generator_pool),30)):
        ...
        for _,r in pool_rotated.iterrows():
            ...
            if r["Matchup"] in used_matchups: continue
            selected.append(r); used_matchups.add(r["Matchup"])
        if len(selected)<3:
            for _,r in pool_rotated.iterrows():
                if len(selected)>=3: break
                if r["Player"] not in [x["Player"] for x in selected]: selected.append(r)
        if len(selected)==3:
            names=tuple(sorted([x["Player"] for x in selected]))
            if names not in used_sets:
                used_sets.add(names); combos.append(pd.DataFrame(selected))
    return combos

parlay_hr=smart_hr_parlays(parlay_pool); parlay_hit=tier_parlays(parlay_pool,"Hit %","Hit"); parlay_tb=tier_parlays(parlay_pool,"TB %","TB"); parlay_rbi=tier_parlays(parlay_pool,"RBI %","RBI"); parlay_laser=tier_parlays(parlay_pool,"Laser %","Laser"); parlay_k=tier_parlays(k_df,"Best K%","K","Pitcher") if not k_df.empty else pd.DataFrame()
...
full_cols=["Player","Team","Matchup","Pitcher","Park","Game Status","Parlay Eligible","Lineup","Order","Dinger Score","Grade","Badge","HR %","Hit %","TB %","RBI %","Laser %","Form","Auto Matchup Edge","Pitcher Risk","Park Edge","Weather Edge","Power","Laser","Season HR","HR Rank","Data Source"]
breakdown_cols=["Player","Team","Matchup","Pitcher","Park","Game Status","Parlay Eligible","Dinger Score","HR %","Auto Matchup Edge","Season HR","HR Rank","Data Source","Reasons"]

def render(data,cols=None):
    if data is None or data.empty: return "<div class='note'>No data available.</div>"
    cols=list(data.columns) if cols is None else [c for c in cols if c in data.columns]
    html="<div class='table-wrap'><table class='ai-table'><tr>"+"".join([f"<th>{c}</th>" for c in cols])+"</tr>"
    for _,r in data.iterrows():
        html+="<tr>"
        for c in cols:
            v=r.get(c,""); style=""
            if c=="Grade": style=f"background:{color_grade(v)};font-weight:900;text-align:center;"
            elif c=="Dinger Score": style="background:rgba(34,197,94,.38);font-weight:900;" if safe_float(v)>=28 else "background:rgba(59,130,246,.28);font-weight:900;" if safe_float(v)>=24 else "background:rgba(234,179,8,.22);font-weight:900;"
            elif c=="Parlay Eligible": style="background:rgba(34,197,94,.25);font-weight:900;" if str(v)=="Yes" else "background:rgba(239,68,68,.22);font-weight:900;"
            elif c=="Data Source": style="background:rgba(168,85,247,.25);font-weight:900;" if "Leader" in str(v) else ""
            elif "%" in c or c in ["Avg Model %","Model Combo Confidence","Avg HR %"]: style="background:rgba(34,197,94,.25);" if safe_float(v)>=60 else "background:rgba(234,179,8,.18);" if safe_float(v)>=35 else "background:rgba(239,68,68,.15);"
            elif c=="Form":
            ...
            elif c in ["Reasons","Pick Explanation","Notes","Strategy"]: style="white-space:normal;min-width:520px;color:#cbd5e1;"
            elif c in ["Leg 1 Anchor","Leg 2 Support","Leg 3 Value","Leg 1","Leg 2","Leg 3"]: style="font-weight:800;color:#e5e7eb;"
            html+=f"<td style='{style}'>{v}</td>"
        html+="</tr>"
    return html+"</table></div>"

def pick_card(title,data):
    if data is None or data.empty: return f"<div class='card'><h2>{title}</h2><p>No pick available.</p></div>"
    r=data.iloc[0]
    return f"""<div class='card'><h2>{title}</h2><h3>{r['Player']} — {r['Team']}</h3><p><b>HR %:</b> {r['HR %']}% | <b>Dinger Score:</b> {r['Dinger Score']} | <b>Grade:</b> {r['Grade']} {r['Badge']}</p><p><b>Matchup:</b> {r['Matchup']} vs {r['Pitcher']}</p><p><b>Auto Matchup Edge:</b> {r['Auto Matchup Edge']} | <b>Park:</b> {r['Park']} | <b>Season HR:</b> {r.get('Season HR','N/A')}</p></div>"""
...
with tab4:
    ...
    st.markdown("<div class='note'>Generator builds 3-leg HR combos using high HR %, strong Dinger Score, matchup edge, HR leader boosts, and different games when possible.</div>",unsafe_allow_html=True)
    st.markdown("### 🎰 Clickable Smart HR Parlay Generator")
    if "hr_combo_clicks" not in st.session_state: st.session_state.hr_combo_clicks=0
    if st.button("Generate Different Smart 3-Leg HR Parlay"): st.session_state.hr_combo_clicks += 1
    combos=build_rotating_hr_combos(parlay_pool)
    if combos:
        combo_index=st.session_state.hr_combo_clicks % len(combos); gen_df=combos[combo_index]
        combo_chance=round((safe_float(gen_df.iloc[0]["HR %"])/100)*(safe_float(gen_df.iloc[1]["HR %"])/100)*(safe_float(gen_df.iloc[2]["HR %"])/100)*100,2)
        st.success(f"Smart HR Combo #{combo_index+1} of {len(combos)} | Model Combo Confidence: {combo_chance}%")
        st.markdown(render(gen_df,["Player","Team","HR %","Dinger Score","Grade","Pitcher","Park","Game Status","Auto Matchup Edge","Season HR","Data Source"]),unsafe_allow_html=True)
    else: st.warning("Not enough eligible players to generate different 3-leg HR combos.")
    st.markdown("### 💣 Smart HR Parlays"); st.markdown(render(parlay_hr),unsafe_allow_html=True)
    st.markdown("### ✅ Hit Parlays"); st.markdown(render(parlay_hit),unsafe_allow_html=True)
    st.markdown("### 🧱 Total Bases Parlays"); st.markdown(render(parlay_tb),unsafe_allow_html=True)
    st.markdown("### 🏃 RBI Parlays"); st.markdown(render(parlay_rbi),unsafe_allow_html=True)
    st.markdown("### 🚀 Laser Parlays"); st.markdown(render(parlay_laser),unsafe_allow_html=True)
    st.markdown("### 🎯 Strikeout Parlays"); st.markdown(render(parlay_k),unsafe_allow_html=True)
with tab5:
